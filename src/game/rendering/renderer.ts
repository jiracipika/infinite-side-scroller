 ...
 * Draws sky, terrain, decorations, platforms, particles, and the player.
 */
...
export class GameRenderer {
  ...
  /**
   * Clear and draw the sky gradient.
   */
  ...
  /**
   * Draw parallax background layers (mountains, clouds).
   */
  ...
  private drawMountains(
    ...
    const offsetY = camera.y * parallaxFactor * 0.3;
    ...
    for (let screenX = 0; screenX <= this.width; screenX += 4) {
      ...
      // Simple mountain silhouette with sine waves
  ...
  private drawCloud(x: number, y: number, size: number): void {
    ...
    this.ctx.fill();
    this.ctx.beginPath();
    this.ctx.ellipse(x - size * 0.5, y + size * 0.1, size * 0.6, size * 0.3, 0, 0, Math.PI * 2);
    this.ctx.fill();
    this.ctx.beginPath();
    this.ctx.ellipse(x + size * 0.4, y + size * 0.15, size * 0.5, size * 0.25, 0, 0, Math.PI * 2);
    this.ctx.fill();
  }

  /**
   * Draw terrain for all visible chunks.
   */
  ...
  private drawChunkTerrain(chunk: Chunk, camera: Camera): void {
    ...
    for (const cave of chunk.caves) {
      ...
      ctx.fillStyle = colors.sky; // Cave interior shows sky/background
      ctx.globalAlpha = 0.7;
  ...
  /**
   * Draw decorations (trees, rocks, bushes).
   */
   ...
   */
  drawHUD(player: Player, chunkCount: number): void {
    const ctx = this.ctx;
    ...
    ctx.fillText(`Position: ${Math.round(player.x)}, ${Math.round(player.y)}`, 14, 24);
    ctx.fillText(`Chunks: ${chunkCount}`, 14, 40);
    ctx.fillText(player.onGround ? 'On Ground' : 'In Air', 14, 56);
  }
}
